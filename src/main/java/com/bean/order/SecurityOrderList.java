package com.bean.order;
import java.util.Date;

/**
 * Auto-generated: 2022-01-06 20:26:44
 *
 * @author bejson.com (i@bejson.com)
 * @website http://www.bejson.com/java2pojo/
 */
class SecurityOrderList {

    private String id;
    private String accountNo;
    private String combPositionId;
    private String code;
    private String ddlx;
    private String side;
    private String status;
    private String cxbz;
    private String isStopOut;
    private int wtsl;
    private int cdsl;
    private int cjsl;
    private long cjje;
    private int wtjg;
    private long djje;
    private String htbh;
    private String exchange;
    private int djsl;
    private int yj;
    private int ghf;
    private int jsf;
    private String reason;
    private Date wtsj;
    private Date cjsj;
    private String extra;
    private long qsDay;
    private String stockName;
    private long sxyk;
    private Date updateTime;
    private Date createTime;
    private int unit;
    private String gameId;
    private String stopOutMoney;
    public void setId(String id) {
        this.id = id;
    }
    public String getId() {
        return id;
    }

    public void setAccountNo(String accountNo) {
        this.accountNo = accountNo;
    }
    public String getAccountNo() {
        return accountNo;
    }

    public void setCombPositionId(String combPositionId) {
        this.combPositionId = combPositionId;
    }
    public String getCombPositionId() {
        return combPositionId;
    }

    public void setCode(String code) {
        this.code = code;
    }
    public String getCode() {
        return code;
    }

    public void setDdlx(String ddlx) {
        this.ddlx = ddlx;
    }
    public String getDdlx() {
        return ddlx;
    }

    public void setSide(String side) {
        this.side = side;
    }
    public String getSide() {
        return side;
    }

    public void setStatus(String status) {
        this.status = status;
    }
    public String getStatus() {
        return status;
    }

    public void setCxbz(String cxbz) {
        this.cxbz = cxbz;
    }
    public String getCxbz() {
        return cxbz;
    }

    public void setIsStopOut(String isStopOut) {
        this.isStopOut = isStopOut;
    }
    public String getIsStopOut() {
        return isStopOut;
    }

    public void setWtsl(int wtsl) {
        this.wtsl = wtsl;
    }
    public int getWtsl() {
        return wtsl;
    }

    public void setCdsl(int cdsl) {
        this.cdsl = cdsl;
    }
    public int getCdsl() {
        return cdsl;
    }

    public void setCjsl(int cjsl) {
        this.cjsl = cjsl;
    }
    public int getCjsl() {
        return cjsl;
    }

    public void setCjje(long cjje) {
        this.cjje = cjje;
    }
    public long getCjje() {
        return cjje;
    }

    public void setWtjg(int wtjg) {
        this.wtjg = wtjg;
    }
    public int getWtjg() {
        return wtjg;
    }

    public void setDjje(long djje) {
        this.djje = djje;
    }
    public long getDjje() {
        return djje;
    }

    public void setHtbh(String htbh) {
        this.htbh = htbh;
    }
    public String getHtbh() {
        return htbh;
    }

    public void setExchange(String exchange) {
        this.exchange = exchange;
    }
    public String getExchange() {
        return exchange;
    }

    public void setDjsl(int djsl) {
        this.djsl = djsl;
    }
    public int getDjsl() {
        return djsl;
    }

    public void setYj(int yj) {
        this.yj = yj;
    }
    public int getYj() {
        return yj;
    }

    public void setGhf(int ghf) {
        this.ghf = ghf;
    }
    public int getGhf() {
        return ghf;
    }

    public void setJsf(int jsf) {
        this.jsf = jsf;
    }
    public int getJsf() {
        return jsf;
    }

    public void setReason(String reason) {
        this.reason = reason;
    }
    public String getReason() {
        return reason;
    }

    public void setWtsj(Date wtsj) {
        this.wtsj = wtsj;
    }
    public Date getWtsj() {
        return wtsj;
    }

    public void setCjsj(Date cjsj) {
        this.cjsj = cjsj;
    }
    public Date getCjsj() {
        return cjsj;
    }

    public void setExtra(String extra) {
        this.extra = extra;
    }
    public String getExtra() {
        return extra;
    }

    public void setQsDay(long qsDay) {
        this.qsDay = qsDay;
    }
    public long getQsDay() {
        return qsDay;
    }

    public void setStockName(String stockName) {
        this.stockName = stockName;
    }
    public String getStockName() {
        return stockName;
    }

    public void setSxyk(long sxyk) {
        this.sxyk = sxyk;
    }
    public long getSxyk() {
        return sxyk;
    }

    public void setUpdateTime(Date updateTime) {
        this.updateTime = updateTime;
    }
    public Date getUpdateTime() {
        return updateTime;
    }

    public void setCreateTime(Date createTime) {
        this.createTime = createTime;
    }
    public Date getCreateTime() {
        return createTime;
    }

    public void setUnit(int unit) {
        this.unit = unit;
    }
    public int getUnit() {
        return unit;
    }

    public void setGameId(String gameId) {
        this.gameId = gameId;
    }
    public String getGameId() {
        return gameId;
    }

    public void setStopOutMoney(String stopOutMoney) {
        this.stopOutMoney = stopOutMoney;
    }
    public String getStopOutMoney() {
        return stopOutMoney;
    }

    @Override
    public String toString() {
        return "SecurityOrderList{" +
                "id='" + id + '\'' +
                ", accountNo='" + accountNo + '\'' +
                ", combPositionId='" + combPositionId + '\'' +
                ", code='" + code + '\'' +
                ", ddlx='" + ddlx + '\'' +
                ", side='" + side + '\'' +
                ", status='" + status + '\'' +
                ", cxbz='" + cxbz + '\'' +
                ", isStopOut='" + isStopOut + '\'' +
                ", wtsl=" + wtsl +
                ", cdsl=" + cdsl +
                ", cjsl=" + cjsl +
                ", cjje=" + cjje +
                ", wtjg=" + wtjg +
                ", djje=" + djje +
                ", htbh='" + htbh + '\'' +
                ", exchange='" + exchange + '\'' +
                ", djsl=" + djsl +
                ", yj=" + yj +
                ", ghf=" + ghf +
                ", jsf=" + jsf +
                ", reason='" + reason + '\'' +
                ", wtsj=" + wtsj +
                ", cjsj=" + cjsj +
                ", extra='" + extra + '\'' +
                ", qsDay=" + qsDay +
                ", stockName='" + stockName + '\'' +
                ", sxyk=" + sxyk +
                ", updateTime=" + updateTime +
                ", createTime=" + createTime +
                ", unit=" + unit +
                ", gameId='" + gameId + '\'' +
                ", stopOutMoney='" + stopOutMoney + '\'' +
                '}';
    }
}